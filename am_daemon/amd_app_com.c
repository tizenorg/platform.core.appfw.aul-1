...
int app_com_broker_init()
{
	...
	if (!cpid_tbl) {
		cpid_tbl = g_hash_table_new(g_int_hash, g_int_equal);
		if (cpid_tbl == NULL) {
			_E("out of memory");
			return -1;
		}
...
int app_com_add_endpoint(const char *endpoint, unsigned int propagate, const char *assoc_priv)
{
	...
	info = (struct endpoint_info *)g_malloc(sizeof(struct endpoint_info));
...
static void __add_client(struct endpoint_info *info, const char *filter, int pid)
{
	...
	c = (struct client_info *)g_malloc(sizeof(struct client_info));
	c->endpoint = info;
	c->pid = pid;
	if (filter)
...
int app_com_send(const char *endpoint, int cpid, bundle *envelope)
{
	...
	bundle_add_byte(envelope, AUL_K_COM_RESULT, GINT_TO_POINTER(AUL_APP_COM_R_OK), sizeof(AUL_APP_COM_R_OK));

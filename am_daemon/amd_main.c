...
gboolean  __amd_ready(gpointer user_data)
{
	...
	fd = creat("/tmp/amd_ready_tmp", S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH);

	if (fd == -1) {
		_E("failed to create /tmp/amd_ready_tmp: %s\n",
			strerror(errno));
		return FALSE;
	}

	/* /tmp/amd_ready file is for notifying that the amd is ready,
	   So, it is labeld to '_' intentionally. */
	ret = fsetxattr(fd, "security.SMACK64", "_", 1, 0);
	if (ret == -1) {
		_E("failed to set _ label to /tmp/amd_ready_tmp: %s\n",
			strerror(errno));
		close(fd);
		return FALSE;
	}

	/* rename after setxattr to make the creation of /tmp/amd_ready with
	   label '_' to be atomic */
	ret = rename("/tmp/amd_ready_tmp", "/tmp/amd_ready");
	if (ret == -1)
		_E("failed to create /tmp/amd_ready: %s\n", strerror(errno));

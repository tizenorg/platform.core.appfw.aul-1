...
/**
 * @par Description:
 * This function sets the default applicationi(package name) associated with operatioin, uri and mime-type.
 *
 * @param[in] b Bundle object Target Package name and operation, uri and mime-type.
 *
 * @return 0 if success, negative value(<0) if fail
 * @see None
 * @remarks This API is only for Appfw internally.
...
...
{
    ...
    const char *pkg_name = "test_appid";
    ...
    aul_svc_set_appid(b, pkg_name)

    r = aul_set_default_app_byoperation(b);
}

 * @endcode
 **/
int aul_set_default_app_byoperation(bundle *b);
#ifdef __cplusplus
	}
#endif

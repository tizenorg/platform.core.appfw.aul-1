...
int aul_socket_send_result_with_fd(int fd, int res)
{
	int ret;
	...
	ret = send(fd, &res, sizeof(int), MSG_NOSIGNAL);
	if (ret < 0) {
		if (errno == EPIPE)
...
int aul_socket_send_raw_async_with_fd(int fd, int cmd,
 ...
 */
int aul_socket_send_raw(int pid, uid_t uid, int cmd,
		unsigned char *kb_data, int datalen)
{
	int fd;
	int len;
...
app_pkt_t *aul_socket_recv_pkt(int fd, int *clifd, struct ucred *cr)
{
	int len;
	int ret;

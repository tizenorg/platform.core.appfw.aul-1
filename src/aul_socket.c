...
API int aul_socket_create_server(int pid, uid_t uid)
{
...
API int aul_socket_send_raw_async_with_fd(int fd, int cmd,
 ...
 */
API int aul_socket_send_raw(int pid, uid_t uid, int cmd,
		unsigned char *kb_data, int datalen)
{
	int fd;
	int len;
	...
	res = aul_socket_send_raw_async_with_fd(fd, cmd, kb_data, datalen);
	if (res < 0) {
		close(fd);
		return res;
	}
...
API int aul_socket_send_raw_async(int pid, uid_t uid, int cmd, unsigned char *kb_data, int datalen)
{
	int fd;
	...
	aul_socket_send_raw_async_with_fd(fd, cmd, kb_data, datalen);

	return fd;
}

API app_pkt_t *aul_socket_recv_pkt(int fd, int *clifd, struct ucred *cr)
{
	int len;
	int ret;
...
API app_pkt_t *aul_socket_send_raw_with_pkt_reply(int pid, uid_t uid, int cmd, unsigned char *kb_data, int datalen)
{
	int fd;
	int len;
	int ret;
	...
	ret = aul_socket_send_raw_async_with_fd(fd, cmd, kb_data, datalen);
	if (ret < 0) {
		close(fd);
		return NULL;
	}

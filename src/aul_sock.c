...
#endif
#ifdef TIZEN_FEATURE_CHANGE_SOCK_TIMEOUT_SEC
#include <glib.h>
#include <vconf.h>
#endif
...
static inline void __set_sock_option(int fd, int cli)
{
	...
	tv_sec = g_ascii_strtoull(buf &ptr, 10);
	tv_usec = g_ascii_strtoull(ptr + 1, &ptr, 10);
	tv.tv_sec = tv_sec;
	tv.tv_usec = tv_usec * 1000;
	_D("sec: %.3f, tv_sec: %ld, tv_usec: %ld", sec, tv_sec, tv_usec);
#endif

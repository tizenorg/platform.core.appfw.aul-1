...
static int __app_send_cmd_with_fd(int pid, int uid, int cmd, bundle *kb, int *ret_fd)
{
	...
	if ((res = aul_sock_send_raw_with_fd_reply(pid, uid, cmd, kb_data, datalen, 0, ret_fd)) < 0) {
		switch (res) {
		case -EINVAL:
			res = AUL_R_EINVAL;
			break;
...
{
	return __send_cmd_for_uid_opt(pid, getuid(), cmd, kb, 0);
}

API int app_send_cmd_for_uid(int pid, uid_t uid, int cmd, bundle *kb)
{
	return __send_cmd_for_uid_opt(pid, uid, cmd, kb, 0);
}

...
SLPAPI int app_send_cmd_with_fd(int pid, int uid, int cmd, bundle *kb, int *ret_fd)
{
	int datalen;
...
int app_request_to_launchpad_with_fd(int cmd, const char *appid, bundle *kb, int *fd, int uid)
{
	...
	ret = app_send_cmd_with_fd(AUL_UTIL_PID, uid, cmd, kb, fd);
...
{
	int ret;
	ret = app_request_to_launchpad_with_fd(APP_GET_SOCKET_PAIR, NULL, kb, fd, getuid());
	return ret;
}

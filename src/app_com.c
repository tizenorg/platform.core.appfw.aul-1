...
static void __add_handler(const char *endpoint, app_com_receiver handler, void *user_data)
{
	...
	h = (struct app_com_handler *)g_malloc0(sizeof(struct app_com_handler));
	if (h == NULL) {
		_E("out of memory");
		return;
	}
...
SLPAPI int aul_app_com_create(const char *endpoint, aul_app_com_permission_h permission, app_com_receiver handler, void *user_data)
{
	bundle *b = NULL;
	int ret = 0;

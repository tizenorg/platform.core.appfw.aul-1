...
static void __foreach_cb(const char *key, const int type, const bundle_keyval_t *kv, void *user_data)
{
...
API int aul_widget_instance_foreach(const char *widget_id, aul_widget_instance_foreach_cb cb, void *data)
{
	int ret;
	int fd;
	bundle *kb;
...
extern int app_request_to_launchpad_for_uid(int cmd, const char *appid, bundle *kb, uid_t uid);

API int aul_widget_instance_update(const char *widget_id, const char *instance_id, bundle *param)
{
	int ret;
	bundle *kb = param;
	const char *appid;
	...
	ret = app_request_to_launchpad_for_uid(WIDGET_UPDATE, appid, kb, getuid());

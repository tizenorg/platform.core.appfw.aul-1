...
int __app_send_raw_with_fd_reply(int pid, int cmd, unsigned char *kb_data, int datalen, int *ret_fd)
{
	int fd;
	int len;
	int ret;
...
retry_recv:
	...
	if(cmd == APP_GET_SOCKET_PAIR) {
		...
		vec[0].iov_len = 1024;
		ret = recv_message(fd, vec, 1, &vecsz, desc, 5, &descsz);
		if (ret < 0) {

...
static int __get_pkgid_by_appid(char *pkgid, int pkgid_len, const char *appid, uid_t uid)
{
	...
	// get pkginfo handle
	int err = pkgmgrinfo_appinfo_get_usr_appinfo(appid, &handle, uid);
	if (err != PMINFO_R_OK) {
...
{
	return __get_path_by_appid(path, appid, _SHARED_DATA_DIR, false, getuid());
}

SLPAPI int aul_get_app_shared_resource_path_by_appid(const char *appid, char **path)
{
	return __get_path_by_appid(path, appid, _SHARED_RESOURCE_DIR, false, getuid());
}

SLPAPI int aul_get_app_shared_trusted_path_by_appid(const char *appid, char **path)
{
	return __get_path_by_appid(path, appid, _SHARED_TRUSTED_DIR, false, getuid());
}

SLPAPI int aul_get_app_external_shared_data_path_by_appid(const char *appid, char **path)
{
	return __get_path_by_appid(path, appid, _SHARED_DATA_DIR, true, getuid());
}

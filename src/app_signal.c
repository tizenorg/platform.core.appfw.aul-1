                                        ...
                                        gpointer user_data)
{
        ...
        if (strcmp(signal_name, AUL_DBUS_APPDEAD_SIGNAL) == 0) {
                g_variant_get(parameters, "(u)", &pid);

                if (app_dead_handler)
                        app_dead_handler((int)pid, user_data);
        } else if (strcmp(signal_name, AUL_DBUS_APPLAUNCH_SIGNAL) == 0) {
                g_variant_get(parameters, "(us)", &pid, &appid);

                if (app_launch_handler)
                        app_launch_handler((int)pid, user_data);

                if (app_launch_handler2)
                        app_launch_handler2((int)pid,
                                        (const char *)appid, user_data);

                g_object_unref(appid);
        }
}
                                        ...
                                        gpointer user_data)
{
        ...
        if (strcmp(signal_name, SYSTEM_SIGNAL_BOOTING_DONE) == 0) {
                if (booting_done_handler)
                        booting_done_handler((int)pid, user_data);
        } else if (strcmp(signal_name, RESOURCED_SIGNAL_PROCESS_STATUS) == 0) {
                g_variant_get(parameters, "(ii)", &status, &pid);

                if (status_handler)
                        status_handler((int)pid, (int)status, user_data);
        } else if (strcmp(signal_name, SYSTEM_SIGNAL_COOLDOWN_CHANGED) == 0) {
                ...
                g_object_unref(cooldown_status);
        }
}

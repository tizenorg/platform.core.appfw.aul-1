...
static gboolean fast_run_func(void *data)
{
	...
	snprintf(path, sizeof(path), "/run/user/%d/%s", getuid(), AMD_READY);
	if (access(path, F_OK) == 0) {
		pid = aul_launch_app(launch_arg_data->appid, kb);
	} else {
		...
		pid = app_send_cmd_to_launchpad(LAUNCHPAD_PROCESS_POOL_SOCK, getuid(),
				0, kb);
	}
	if (pid > 0) {

...
static int __set_appinfo_for_launchpad(bundle *kb, const char *appid)
{
	...
	if (menu_info->component_type
		&& strcmp(menu_info->component_type, APP_TYPE_SERVICE) != 0
	...
	bundle_add(kb, AUL_K_HWACC, menu_info->hwacc);
	...
	bundle_add(kb, AUL_K_INTERNAL_POOL, menu_info->process_pool);
